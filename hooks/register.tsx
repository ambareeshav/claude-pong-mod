...
// Pong above the Claude Code prompt, built the same way as the tetris mod: `/pong` opens it,
// `/pong stop` closes it. The board in ./boards is a surface module that runs on the drawing
// thread; this module mounts it, keeps the best winning margin in $.store and pauses the game
// when Claude finishes a turn. Playing costs no tokens.
...
export const register: Register = on => {
  on('session.start', async ($, e, next) => {
    ...
    // a store that cannot be read costs the best margin, never the game
  ...
  on('command.run', { command: 'pong' }, async ($, e) => {
    ...
    open = true
    $.ui.invalidate('ui.render')
    const score = best ? ` · best margin ${best}$` : ''
  ...
  // a finished match, posted by the board: keep the best margin and hand it back with the next props
  on('ui.message', async ($, e, next) => {
    ...
    if (isBetter(data.score, best || undefined, LOWER_IS_BETTER)) {
      ...
      $.ui.toast(`$Pong: new best margin ${best}$`)
    }
  ...
  on('ui.render', { component: 'AbovePrompt' }, async ($, e, next) => {
    ...
    return (
      <Box flexDirection="column">
        ...
        <Client key="board:pong" module="./boards/pong.tsx" width={cols} height={tall(17)} props={{ best, done: turnsDone, safe: false }} />
        {await next(e)}
      </Box>
    )
  })
